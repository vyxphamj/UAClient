...
// Engineering: Visual Studio 2018
...
// 01.04.00 27.11.2018 (Siemens) Update to OPC UA stack V1.04; Minor bug fixes
//=============================================================================
...
namespace Siemens.UAClientHelper
{
    public class UAClientHelperAPI
    {
        ...
        public UAClientHelperAPI()
        {
            ...
            //configuration.ApplicationName = "Vyxphamj's Client";
            //configuration.ApplicationUri = "urn:localhost:OPCFoundation:Vy_Client";
            //configuration.SecurityConfiguration.ApplicationCertificate.SubjectName =configuration.ApplicationName;
            //configuration.ApplicationType = ApplicationType.Client;
            //IList<string> domainNames = new List<string>();
            //ushort keySize = 2048;
            //DateTime startTime = DateTime.Parse("2022-01-01");
            //ushort lifetimeInMonths = 24;
            //ushort hashSizeInBits = 256;
            //var mycer = CertificateFactory.CreateCertificate(
            ...
            #region DisableHiResClock
            //configuration.DisableHiResClock = true;
            #endregion

            //mApplicationConfig = configuration; 
            #endregion
        }
        #endregion

        #region Properties
        public static bool havecertficate;
        public static X509Certificate2 mycert;
        /// <summary> 
        ...
        public async Task Connect(string url, string secPolicy, MessageSecurityMode msgSecMode, bool userAuth, string userName, string password)
        {
            try
            {
                ...
                mSession = await Session.Create(
                    ApplicationConfig,
                    Endpoint,
                    true,               //We need to update the server's endpoint to get available certificate and identity tokens
                    "MySession",
                    60000,
                    UserIdentity,
                    null
        ...
        private static ApplicationConfiguration CreateClientConfiguration()
        {
            ...
            //string applicationUri = "urn:vyxphamj:OPCFoundation:SampleClient";
            //string applicationName = "Vyxphamj";
            //string subjectName = "Vyxphamj's Client";
            //IList<string> domainNames = new List<string>();
            //var myIbuilder = CertificateFactory.CreateCertificate(applicationUri, applicationName, subjectName, domainNames);
            //myIbuilder.SetNotAfter(DateTime.Parse("2022-01-01"));
            //myIbuilder.SetNotBefore(DateTime.Parse("2021-12-29"));
            //myIbuilder.SetRSAKeySize((ushort)2048);
            //myIbuilder.SetHashAlgorithm(new HashAlgorithmName("SHA256"));
            //var certification = myIbuilder.CreateForRSA();

            //Console.WriteLine("NotBefore: " + certification.NotBefore);
            //Console.WriteLine("NotAfter: " + certification.NotAfter);
            //Console.WriteLine("KeySize: " + certification.PublicKey.Key.KeySize);
            //Console.WriteLine("Version: " + certification.Version);
            //Console.WriteLine("SerialNumber: " + certification.SerialNumber);
            //Console.WriteLine("SignatureAlgorithm: " + certification.SignatureAlgorithm.FriendlyName);
            //Console.WriteLine("Issuer: " + certification.Issuer);
            //Console.WriteLine("Subject: " + certification.Subject);
            //Console.WriteLine("Thumprint: " + certification.Thumbprint);

            //byte[] certData = certification.Export(X509ContentType.Cert);
            //File.WriteAllBytes(@"E:\CertOfVyxPhamj.cer", certData); 
            #endregion
            ...
            configuration.ApplicationType = ApplicationType.Client;
            configuration.ApplicationUri = "urn:VyxPhamj:UAClient"; //Kepp this syntax
            configuration.ProductUri = Utils.Format(@"urn:" + configuration.ApplicationName);
            ...
            configuration.SecurityConfiguration.MinimumCertificateKeySize = 128;
            ...
            {
                case "Android":
                    currentFolder = DependencyService.Get<IPathService>().PublicExternalFolder.ToString();
                    own = Path.Combine(currentFolder, "UA_Client/pki/own");
                    configuration.SecurityConfiguration.ApplicationCertificate.StoreType = CertificateStoreType.Directory;
                    configuration.SecurityConfiguration.ApplicationCertificate.StorePath = own;
                    configuration.SecurityConfiguration.ApplicationCertificate.SubjectName = "CN=UA Client, C=VN, S=HoChiMinhCity, O=HCMUT, DC=localhost";

                    // Define trusted root store for server certificate checks
                    issuer = Path.Combine(currentFolder, "UA_Client/pki/issuer");
                    configuration.SecurityConfiguration.TrustedIssuerCertificates.StoreType = CertificateStoreType.Directory;
                    configuration.SecurityConfiguration.TrustedIssuerCertificates.StorePath = issuer;
                    trusted = Path.Combine(currentFolder, "UA_Client/pki/trusted");
                    configuration.SecurityConfiguration.TrustedPeerCertificates.StoreType = CertificateStoreType.Directory;
                    configuration.SecurityConfiguration.TrustedPeerCertificates.StorePath = trusted;

                    // Define rejected certificate store
                    rejected = Path.Combine(currentFolder, "UA_Client/pki/rejected");
                    configuration.SecurityConfiguration.RejectedCertificateStore.StoreType = CertificateStoreType.Directory;
                    configuration.SecurityConfiguration.RejectedCertificateStore.StorePath = rejected;
                    break;
            ...
            if (clientCertificate.Result == null)
            {
                ...
                DateTime startTime = System.DateTime.Now.AddDays(-2);
                ...
                X509Certificate2 clientCertificate2 = CertificateFactory.CreateCertificate(
                    ...
                    lifeTime,
                    algorithm);
                configuration.SecurityConfiguration.ApplicationCertificate.Certificate = clientCertificate2;
            }
            ...
            {
                havecertficate = false;
            }
            
            
            mycert = configuration.SecurityConfiguration.ApplicationCertificate.Certificate;
